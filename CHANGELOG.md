...
- **The watchdog paged for 44 days about a service that had been deleted.** The meta-search subsystem was removed on 2026-07-02, along with its scheduled jobs, but its liveness row stayed in the database — and the watchdog alerts on any service it has ever seen that stops reporting, with no notion of one being retired on purpose. Because that job was not tied to market hours, it paged around the clock. Retired services are now ignored by name; the row is left in place so the retirement stays visible rather than being quietly erased.
