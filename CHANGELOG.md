...
- **`exit_decisions` now records which exit rule actually fired.** Every exit routed through the monitor was relabelled `ml_exit_<bucket>` with `IMMEDIATE` urgency before being persisted (all 39 July rows), so stop-loss, profit-target, expiry, and flatten exits were indistinguishable in the audit trail and in the nightly bucket metrics. The rule's own `rule_id` and urgency are persisted, and the row is linked to its entry via `candidate_id` (previously always NULL) so per-rule metrics and the position manager's open-position reconciliation can join exits back to entries. Classifier-driven exits keep the `ml_exit_<bucket>` label.
