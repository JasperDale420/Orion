...
- **The e2e smoke test's execution stage no longer fails when CI happens to run outside regular trading hours.** Its mock order went through `ExecutionEngine`'s real sizing math, which scales the fixed per-trade budget by RegimeGate's session multiplier (premarket/close sessions score as low as 0.42-0.5x). At that reduced budget the test's option price rounded down to 0 contracts, which short-circuits before the mock broker is ever called — failing the stage's `persist_pending_order` assertion. The regime sizing multiplier is now pinned to a fixed value before the execution stage runs, so the stage's pass/fail no longer depends on what time of day the test happens to run (verified against a real TimescaleDB with both a mocked premarket and a regular session).
- **Positions bought 1 day before expiry are no longer dumped minutes after entry.** The expiry time stop measured time-to-expiry in wall-clock hours and compared that fraction against the bucket's whole-day `min_dte` floor, while the entry rules, the bucket classifier and the per-bucket entry caps all count calendar days. A Wednesday-expiring contract bought Tuesday is a legitimate 1-DTE SHORT_SWING entry, but the exit rule read it as "0.43 days remaining ≤ min_dte=1" and closed it on the very next monitor cycle — on 2026-08-18 that exited five positions 1 to 12 minutes after entry (NVDA, META ×2, QQQ, TSLA) for a combined −$83 of pure bid/ask friction. The time stop now counts calendar days from the current New York date and exits only when *fewer* than `min_dte` days remain: SHORT_SWING exits on expiry day, SWING and POSITION exit with one day left, and 0DTE is unchanged (its deadline is the 15:45 ET flatten). Positions with no recorded expiry keep the existing no-op plus the contract-symbol fallback, so none is left without a time stop, and the deadline is snapped to the last actual trading session so a contract dated on a weekend or exchange holiday still exits on the last day it can be sold. If the exchange calendar is ever unavailable the time stop keeps working on a weekday-only deadline and raises one alert on the way into and out of that degraded state.
...
- **A shadow factor set is logged on every executed candidate**, so the trading ideas from the options literature can finally be scored against Orion's *own* realized P&L instead of a third-party label. The recorded factors are: prior 24-hour signed flow agreement on the same underlying, 20-day realized volatility, the realized-versus-implied volatility spread, the volatility tilt that separates calls from puts, absolute delta, volatility-standardized moneyness, days to expiry, premium, quoted spread, and the DTE bucket. They are measured strictly from information that existed before the candidate fired — prior flow is cut off at the candidate's own timestamp, and daily closes stop before the session it fired in — so a value can be reproduced from the stored trace weeks later. Each factor is written to `decision_trace_json.factors` and logged as one `candidate_factors` event. A factor that cannot be computed is recorded as missing rather than guessed, and a database that is slow or down costs the factor, never the order.
- **`ORION_FACTOR_GATES` can promote any of those factors from a measurement into an entry filter** — for example `{"f_vrp": {"min": -0.5}, "f_dte": {"max": 30}}` skips candidates whose volatility spread or expiry falls outside those bands, recorded on the decision as `Factor gate: …` like any other skip. It is **empty by default**, which is deliberate: none of these factors has evidence behind it yet, so out of the box they only watch. A factor that could not be measured never blocks a trade.

### Fixed

- **A fill's effect on the account can no longer land twice, vanish, or get stuck behind a "done" marker that isn't actually done — across five distinct failure paths, closed one at a time as each was found by adversarial review.** `FillProcessor` used to record a fill as "processed" and tell the risk manager about it as two separate database writes; if the second one failed after the first succeeded, a retry either got stuck forever (blocked by its own in-memory bookkeeping) or, after a restart, re-told the risk manager about a fill it had already durably counted — double-booking that trade's profit or loss. The risk manager had a matching version of the same bug internally: it flagged a fill as "already seen" before actually saving the account-wide numbers, so a save failure could leave a fill's effect vanished from equity, position size, and the options-Greeks risk tracking with no way to retry, or (separately) leave a new equity high unsaved, understating how bad a later loss looks against the true peak. All of these are now impossible by construction: the fill row, the account-state update, and the "done" marker are written as one all-or-nothing database transaction — either everything lands or nothing does, so there's no window where the system can be lying about what it actually recorded. The one remaining edge case — the database write succeeds but the confirmation gets lost in transit before this process hears back — is handled by checking, after every retry (including the very last one), whether the write actually landed and picking up from there instead of assuming it failed. The only gap left un-closed, by explicit design choice: if the "stop all new trades" breaker's own database write fails during an active loss event, this process still refuses new trades on its own (that part isn't touched), but a *different* process or a fresh restart wouldn't yet see the stop — a narrower, pre-existing reliability gap, not something this fix could safely close without a larger, separate piece of work.
- **The market-shock trading gate no longer trusts an ETF-price guess as if it were real VIX.** Enabling `ORION_FEATURE_ENRICHMENT_PREFER_HEBER_CONTEXT` (previously inert, see the entry below from earlier this week) turned out to wire a VIXY-close-times-two approximation into the same hard "block every new trade" path documented for a genuine VIX spike — tested live on 2026-08-18, a real VIXY close of $18.83 was read as "VIX 37.66 / SHOCK" and immediately stopped all new entries. `RegimeGate` now only honors that hard block when the reading is tagged as a trusted spot-VIX source; every source that exists today (the VIXY/UVIX/VIXM proxy) is untagged, so the hard block is inert again until a real VIX feed is wired in — but an untrusted SHOCK reading still raises an operator-visible `proxy_would_block` alert (once per onset, not once per candidate) instead of disappearing silently. A snapshot's vix reading is also no longer trusted just because the row was recently rewritten: the underlying observation's own timestamp must independently be within the 15-minute freshness window, closing a gap where a live process could re-persist the same stale close every ~5 minutes and look perpetually current. `ORION_FEATURE_ENRICHMENT_PREFER_HEBER_CONTEXT` now also defaults to off at the settings level (previously only Docker Compose's own separate default kept it off), so a raw, non-Compose launch can no longer silently re-enable this path.
- **Regime-based position sizing has silently been a no-op since this module's first commit (2026-01-02).** `RegimeRiskManager`'s config loader computed a path three directories short of `config/regime_risk.yaml`, so it always read a file that doesn't exist and silently fell back to `{}` — every vol/risk/vix/session sizing multiplier defaulted to `1.0` regardless of market regime, for this module's entire life. The only regime-based trading control that ever actually worked was the hard SHOCK block itself. Fixed one path segment; regime-based sizing now actually applies (see below for the fix that lets it reach live order sizes).
- **Regime-based sizing reduction now actually reaches real orders.** The computed regime multiplier above was already being read by `ExecutionEngine`, but only in one of its three sizing branches — `risk_per_trade_bps` fallback sizing. The branch that's actually deployed live, fixed-$500-per-trade sizing, silently ignored it entirely, so every market regime (SHOCK, RISK_OFF, calm) sized identically. All three branches now apply it: whichever base a branch selects is capped at the account-level safety ceiling first, then the regime multiplier scales what remains, so an adverse regime shrinks size and a favorable one can reduce toward (never past) the ceiling but never exceed the operator-configured base — a combined multiplier above 1.0 (config permits e.g. LOW vol + RISK_ON + LOW vix all at once) is clamped to 1.0 rather than sizing a trade bigger than configured. A `regime_size_multiplier` that's present but malformed (non-numeric, NaN, infinite, negative, or a boolean) now skips the order and logs the failure instead of silently defaulting to full size — a stale or corrupted value at this boundary could otherwise erase an intended risk reduction without anyone noticing. A genuinely absent multiplier (no regime data computed for this decision) still defaults to full, unmultiplied size, matching the pipeline's own default.
- **Execution service startup no longer opens 7 out of every 8 files it doesn't need, ballooning memory to 1.7 GB before a single candidate is processed.** At startup, `SignalEngine` warms up its indicator history by reading 5 days of 1-minute bars for the watchlist (11 tickers) from the shared Heber parquet cache. That cache keeps every file ever synced, hive-partitioned first by instrument type (equity/crypto/option) and then by day — but the reader's date-based pruning walked every instrument type's folders for the matching days before filtering, not just the one being asked for. The crypto bars partition writes thousands of tiny per-write files a day that never get compacted into daily files (equity's does), so a single equity ticker's 5-day read was opening ~9,375 files — almost all of them crypto files that could never match — to find the ~1,300 equity files it actually needed. Measured before the fix: one ticker took 4.0s and ~680 MB; all 11 tickers together took 29s and drove the process to ~1.18 GB. The reader now narrows its search to the one instrument-type folder being asked for before opening anything, which is always safe — a file under a different instrument type could never contain a matching row. After the fix: one ticker takes 0.7s and ~137 MB; all 11 together take 7.4s and ~390 MB — roughly a 4x speed and 3x memory improvement on this step, with byte-identical results. No trading logic changed; this only touches how historical bars are read off disk. The same overhead affects every other feed this reader serves (flow, darkpool, market tide, greek exposure, IV rank) as their instrument-type partitions grow.
- **"We chose not to trade" is no longer recorded as "the trade failed".** The execution engine already separates a business-reason skip (illiquid quote, earnings window, bucket or underlying position cap, an unavailable position-cap check, the new factor gate) from a genuine failure (risk rejection, option price fetch failure, high error rate, missing expiration date) — but the execution loop overwrote that with a flat `FALSE` before saving, so every skip landed in `strategy_decisions.executed_successfully` looking like a failed execution and no dashboard or research query could tell the two apart. The engine's own verdict is now what gets stored: `SKIPPED` for a skip, `FALSE` for a failure, `TRUE` for a submitted order. Anything else — an attempt the engine returned from without resolving, or an exception thrown out of it — is still recorded as `FALSE`, deliberately, so it can never be left as `PENDING`, which startup reconciliation can only repair from a linked broker order that an unresolved attempt does not have. Rows written before this change still read `FALSE` for both cases and cannot be reclassified retroactively.
- **The position monitor no longer scans a year of market history to compute features nothing reads.** Recovering a position's entry context called the full ML scoring enricher, which builds roughly sixty features — a 370-day bars scan, 365 days of max-pain and unfiltered flow, a week of darkpool, greeks, regimes, multi-timeframe flow windows. The exit classifier's feature vector carries exactly four of them: IV rank, VIX, gamma exposure and market tide at entry. Everything else was computed and thrown away, at a measured cost of 86 Heber reads, ~145 seconds and a 7.26 GB peak **per position**, on the daemon responsible for getting out of trades. Two changes: the enrichment is now skipped unless the position's own expiry bucket has a trained exit model — it feeds only the classifier, and `evaluate_exits` only builds that vector for a bucket that has a model, so with none deployed (the state since 2026-07-01) these scans reached no consumer at all — and when a model *is* loaded, only the four consumed features are resolved. A position whose enrichment was skipped is re-resolved automatically if a model for its bucket is loaded later, and the classifier is withheld from any position whose entry context is still resolving — it substitutes defaults for missing entry features (IV rank 50, VIX 20), so scoring mid-resolution could fire an ML exit on values the position never had. The deterministic barriers stay in force throughout — they are evaluated first and independently — so a deferred position is still being watched for its stop, target, time stop and 0DTE flatten; what is withheld is the model's opinion, which for an exit only the model would have found does mean a real (if narrow) reduction in coverage while it lasts. Withholding is unconditional: even on the rare path where barrier evaluation itself fails and the classifier would normally be the last resort, an unresolved context pages `EXIT_POLICY_EVALUATION_FAILED` (once per position, stating which of the two outcomes actually applies) rather than letting a decision be made on placeholder values. A lookup that fails outright — a data-source outage, say — now counts as unresolved rather than as an empty-but-successful result, so it keeps retrying instead of quietly presenting a position as ready with every entry feature missing. Deferral is normally seconds; if entry context stays unresolvable — a sustained database outage, say — the position is reported every cycle with how long its model has been withheld (`exit_classifier_deferred_context_prolonged`) instead of the condition passing silently. The point-in-time gamma-exposure lookup also no longer drags in a 20-day rolling-average scan whose result was discarded. That drops a resolution from ~145 s / 7.26 GB to **4.0 s / 1.18 GB**, with the model seeing an identical feature vector; with no model loaded it is zero reads. Startup says which mode is in effect (`entry_context_enrichment_disabled_no_exit_models`). Buckets, expiries, entry times and the deterministic exit barriers are derived from the decision row and the contract symbol, not from this enrichment, so exit behaviour is unchanged either way.
- **The position monitor no longer freezes itself solid while looking up a position's entry context — the cause of the 2026-08-17 runaway (4.8 GB, ~74% CPU, exits evaluated 2–7× less often than configured).** Recovering a position's entry context enriches it through the shared flow enricher, which fans out to Heber parquet scans covering up to a year of history — and those scans are ordinary blocking reads that were being awaited directly on the monitor's event loop. Measured against live data: one such enrichment blocks for 166 seconds, during which the loop ticks **once**. Everything else in the cycle — exit evaluation included — is stopped dead for the duration. It also made the 2-second budget meant to bound the lookup unenforceable, because a blocked loop cannot fire its own timers: on 2026-08-17 that "2-second" timeout was measured raising after 85.7 seconds. And because a lookup that overruns its budget is deliberately never cached (so a wrong bucket can't be frozen in), the entire scan cascade re-ran every cycle for every position, forever — 714 times in one session, none of which ever resolved. The result was a monitor that spent its whole day re-computing a result it always threw away, while the exit-evaluation cadence decayed from the configured 60 seconds to 110–420 seconds and still climbing. Entry-context lookups now run on a worker thread, so the monitor loop stays responsive throughout (6,069 ticks across the same 145-second scan, versus 1); a lookup that overruns the cycle budget is no longer cancelled but left to finish and cache, so it costs one scan per symbol per session instead of one per symbol per cycle; only one runs at a time, since a single 370-day bars read peaks above 7 GB — and that one-at-a-time slot is held until the worker genuinely finishes, so a position closing mid-scan cannot hand the slot to a second scan while the first is still resident in memory (a lookup outliving its position is left to drain and its result discarded, rather than cancelled, because a worker thread cannot actually be interrupted); and a lookup that fails outright backs off exponentially (30 s doubling to a 5-minute cap) rather than retrying hot. A position whose context is still resolving keeps the bucket, expiry, and stops derived from its own contract symbol — so a 0DTE gets 0DTE cadence and flatten immediately, never waiting on the lookup — and picks up the real context on the first cycle after it lands. Exit rules, exit routing, and the close path are untouched. Every cycle now also logs the process's peak RSS (`rss_mb`) alongside the position count, so this class of growth is visible in the structured logs instead of only in `top`. Note for anyone reading old logs: the pre-existing tests missed all of this because they simulated a slow lookup with `await asyncio.sleep(...)`, which politely yields to the loop; real parquet I/O does not.
- **The stale-entry cancel sweep now confirms an order is still open at the broker before cancelling it.** On 2026-08-17 entry order d67ea8ac (PLTR260821C00182500) filled at 14:27:35Z, but the sweep's cancel — based on Orion's own not-yet-updated database — went out at 14:29:11Z, one second before the fill was recovered. Alpaca rejected the cancel as "already filled," and the Gateway's ownership guard treated that rejected cancel as an ambiguous account mutation and froze the symbol, blocking every subsequent order on it including Orion's own closes; seven symbols froze that day. Before sending a cancel, the sweep now refreshes the order's real broker status (one batched open-orders lookup covers a whole sweep; only an order absent from that list gets an individual follow-up check). An order the broker reports as filled, canceled, expired, rejected, done-for-day, replaced, suspended, or stopped is reconciled instead of cancelled — the same recovery path already used when a cancel comes back rejected for this reason, and now also recovering any already-executed quantity regardless of which terminal state it lands in (a partial fill followed by a cancel/expiry used to vanish uncounted). A legacy pre-2026-05-20 order the Gateway can never confirm ownership of is reconciled out instead of failing this refresh forever. If the refresh itself fails, the cancel is skipped for that cycle and retried on the next sweep rather than sent unconfirmed. An order still genuinely open but already carrying an executed quantity (a partial fill the database hasn't caught up on) has that quantity recovered before its remainder is cancelled — and if that recovery itself fails, the cancel is deferred to the next sweep rather than risk losing the quantity for good. The remaining race — the broker finalizes an order in the instant between this check and the cancel actually arriving — is addressed by a companion fix on the Data-Gateway side.
- **The stale-entry cancel sweep now confirms an order is still open at the broker before cancelling it.** On 2026-08-17 entry order d67ea8ac (PLTR260821C00182500) filled at 14:27:35Z, but the sweep's cancel — based on Orion's own not-yet-updated database — went out at 14:29:11Z, one second before the fill was recovered. Alpaca rejected the cancel as "already filled," and the Gateway's ownership guard treated that rejected cancel as an ambiguous account mutation and froze the symbol, blocking every subsequent order on it including Orion's own closes; seven symbols froze that day. Before sending a cancel, the sweep now refreshes the order's real broker status (one batched open-orders lookup covers a whole sweep; only an order absent from that list gets an individual follow-up check). An order the broker reports as filled, canceled, expired, rejected, done-for-day, replaced, suspended, or stopped is reconciled instead of cancelled — the same recovery path already used when a cancel comes back rejected for this reason, and now also recovering any already-executed quantity regardless of which terminal state it lands in (a partial fill followed by a cancel/expiry used to vanish uncounted). A legacy pre-2026-05-20 order the Gateway can never confirm ownership of is reconciled out instead of failing this refresh forever. If the refresh itself fails, the cancel is skipped for that cycle and retried on the next sweep rather than sent unconfirmed. An order still genuinely open but already carrying an executed quantity (a partial fill the database hasn't caught up on) has that quantity recovered before its remainder is cancelled — and if that recovery itself fails, the cancel is deferred to the next sweep rather than risk losing the quantity for good. Recovery success is now verified by checking the fill actually landed in the database, not merely that recording it didn't raise an error — the internal fill processor logs and swallows its own failures, so a check that only watched for a raised error could have missed a real failure and cancelled anyway. A recovery that keeps failing no longer defers silently forever with the buying power still reserved — it now goes through the same backoff-then-alert path already used for a rejected cancel, so an operator is paged after repeated failures instead of the order sitting invisibly stuck. The remaining race — the broker finalizes an order in the instant between this check and the cancel actually arriving — is addressed by a companion fix on the Data-Gateway side.
- **The market-shock trading gate can now actually see a shock — but honestly reports that it still can't, today.** `RegimeGate` called the multi-axis regime detector with no VIX, volatility, or market-tide input at all — every candidate was scored against hardcoded defaults, so the documented "block trading in SHOCK / extreme VIX" safeguard could never fire, live or in a backtest. It now does a bounded (5s timeout), cached (60s, capped so the cache can never keep a reading alive past its own 15-minute freshness deadline) read of the latest `regime_snapshots` row `feature_enrichment` already writes every ~5 minutes, and — when that row is fresh (≤15 minutes old, non-negative age so a future-dated/clock-skewed row can't read as "fresh forever", tolerating one missed write cycle) and actually carries a VIX reading — feeds the real vix/realized_vol values into the detector, so a genuine extreme reading now blocks new entries. Failure modes never block: a missing row, a stale or future-dated row, a DB error, or a DB read that hangs past its 5-second timeout (the pipeline awaits each candidate's decision sequentially, so a stuck read would otherwise stall the whole loop) all fall back to exactly today's no-input behavior, logged once as `regime_gate_inert_no_inputs` rather than crashing, blocking, or spamming. Full disclosure of what this fix does *not* do: `regime_snapshots` has never actually carried a real VIX reading in this deployment — every one of 11,000+ rows since 2026-06-27 has `vix_level=NULL`, because `feature_enrichment`'s own Heber-context reads are disabled (`ORION_FEATURE_ENRICHMENT_PREFER_HEBER_CONTEXT=false` in the running container). So the SHOCK gate remains inert in production right now; the decision trace says so explicitly (`regime_inputs: "none"`) instead of implying protection that isn't there. Flipping that upstream flag is a separate, deliberately out-of-scope follow-up — once it's on, this gate starts blocking automatically with no further code change.
- **The smoke-test risk-state snapshot/restore helper can no longer silently drop a column.** It captured and restored `risk_state` through a hardcoded column list that predated `accounting_version` and `daily_loss_date` — a restore would have NULLed both, which fail-closed-blocks all order admission and strips the daily-loss kill switch's session identity. (These helpers are currently only exercised by their own regression test, not by the live smoke run, so this was test-helper hygiene rather than an active production risk.) The helper now selects and inserts every column generically off the `RiskState` table's own schema, so a future column can never be dropped from the round-trip the same way again.
- **The data-quality service's single-instance lease no longer freezes for the rest of the process after a transient database blip.** Its lease-ownership check (part of the 60-second heartbeat) had no error handling, so a brief DB outage (e.g. a Docker VM restart) raised out of the heartbeat loop, silently killing it — the process kept running, but its lease row stopped renewing forever (observed: `service_lease_data_quality` frozen since 01:57Z while the service was still alive). A DB error during the check is now logged (`service_lease_check_failed`) and retried on the next tick rather than crashing the loop; it is never treated as confirmed lease ownership. Confirmed loss of the lease to another owner is unchanged and still fatal. `main_position_monitor.py`'s lease check is deliberately untouched — it is a close-executor and must not run without a verifiable lease.
- **Closing a stuck circuit breaker from the command line works again.** `scripts/reset_circuit_breaker.py` imported `psycopg2`, which is not a project dependency, so the exact command the dead-man-watchdog Discord alert and the circuit-breaker runbook told operators to run failed at import with `ModuleNotFoundError` — leaving no working command-line path to resume trading after a latched breaker. It has been replaced by `uv run python -m orion.jobs.reset_circuit_breaker [--reason "..."] [--dry-run]`, which uses Orion's own async database path instead of a raw driver connection. The admin API's `/admin/circuit-breaker/reset` endpoint now also records the operator-supplied reason instead of always writing a generic one.
- **A heuristically-scored candidate is now judged against the heuristic threshold (0.40), even when other expiry buckets do have a trained model.** ML models are per-bucket, so with (say) only a SWING model deployed, a SHORT_SWING candidate is still scored by the heuristic fallback — but the ML pre-filter picked its threshold from a global "are any models loaded at all" flag, which read "models loaded", and compared that heuristic score against the model-calibrated `ORION_ML_PREFILTER_THRESHOLD` (0.5) instead. Heuristic and model scores are calibrated differently, so a heuristic 0.45 that should have reached the solver ensemble was rejected. The threshold now follows the scoring path that candidate actually took, which the trace already reported as `scoring_mode` — including when a model was loaded for its bucket but the inference attempt itself failed and fell back. Model-scored candidates are unaffected and still face the configured threshold.
- **It's now visible in the decision trace, and in the logs, when a candidate was actually scored by the trained ML model versus the heuristic fallback.** After a recent change made scikit-learn reject incompatible model files instead of loading them with a warning, every score could silently fall back to the heuristic scorer with nothing in `strategy_decisions.decision_trace_json` to show it happened. The ML pre-filter now records `scoring_mode` (`model` or `heuristic`) for the actual outcome of that specific candidate's score — not just whether models are loaded anywhere at all, which could still read "model" for a candidate whose own expiry bucket has no trained model, or whose scoring attempt hit an error and fell back. The scorer also logs one clear error the moment model files exist on disk but every one of them fails to load — distinct from the existing (and unchanged) warning for a directory that legitimately has no trained models in it yet — without re-logging it every minute for a file that hasn't changed.
- **A candidate expiring at the next midnight is now classified as 1-day-to-expiry (SHORT_SWING), not same-day (0DTE).** The ML pre-filter's days-to-expiry calculation subtracted raw timestamps instead of calendar dates, so a candidate signaled a few hours before midnight on an expiry one calendar day out was scored as 0DTE — the wrong bucket, with the wrong ML model and thresholds. It now uses the same calendar-day convention as the rest of the entry path.
- **The entry-pricing chain fetch now asks the Gateway to filter to the one contract being priced.** Orion's live-pricing lookup before order submission requested the Gateway's full option chain for the underlying (hundreds/thousands of contracts on SPY/QQQ) to price the single contract already picked by the signal. It now forwards the candidate's known expiration, option type, and a tight strike window as query params, shrinking the Gateway's response and Orion's parsing work; a candidate missing any one of those fields still gets the prior fully unfiltered fetch, so partial data can never risk excluding the true contract. Note: this does not shorten the Gateway's own upstream Alpaca pagination — that fetch is unchanged and is a separate, cross-repo latency source.
- **A second ingestion or execution process now stops itself instead of quietly writing alongside the first.** Both services heartbeat a single-instance lease, but losing it only produced a `service_lease_lost` log line — the displaced instance carried on writing bronze and silver rows, running the nightly close-of-books, polling fills and submitting orders against the same tables as the winner. That is what a leftover docker ingestion container did on 2026-06-08, thrashing the lease across 345 restarts. A confirmed takeover (the row exists, names another owner, and that owner is still heartbeating) is now fatal: it logs `SERVICE_LEASE_LOST_FENCING` and exits non-zero so the supervisor relaunches, and the relaunched process starts only if the lease is genuinely free. Everything short of proof stays non-fatal exactly as before — a database blip, a missing row, an abandoned lease whose holder has itself gone quiet, or simply being slow to renew our own lease all keep retrying rather than killing a healthy service.
- **The global circuit breaker now means what it says, and stops screaming in the logs.** `ORION_GLOBAL_CIRCUIT_BREAKER_ENABLED=false` — set by the native execution wrapper and by docker-compose — made `is_open()` report a closed breaker while the `GLOBAL_CIRCUIT_BREAKER` row said OPEN. It disabled nothing: the execution engine reads that row directly and blocked every new entry anyway. All it did was make the breaker's own API lie and log `global_circuit_breaker_would_open_but_disabled` once per check — 2,287 times on 2026-08-14 alone. The flag is gone; an OPEN row always halts new entries, reading the breaker is silent, and opening an already-open breaker no longer re-logs the CRITICAL (the drawdown kill switch calls it on every fill). The execution loop's own breaker gate was removed with it: it `continue`d on an open breaker, which also skipped the rule-based exit sweep at the bottom of the same iteration — so a latched breaker would have stopped the halted system from getting flat. The same sweep was also being skipped on every quiet cycle (the loop `continue`d whenever the candidate pool was empty, which is precisely the steady state after a halt); it now runs on every cycle. Exits and closes never consult the breaker; entries are blocked at the two per-candidate gates, each recorded on the decision with `Circuit Breaker Open`. The engine also re-reads the breaker row on every entry instead of trusting its 10-second health cache, so a breaker opened by the operator or by the drawdown kill switch halts entries immediately rather than up to ten seconds later — and closing it resumes them just as promptly.
- **Closed option positions are now recorded as closed in the trade journal, and their real P&L is booked.** The closing-fill write-back looked up the journal row by the fill's OCC contract symbol (`NVDA260708C00190000`) while journal rows carry the underlying (`NVDA`), so no sell fill was ever attributed: every entered lot stayed "open" until the expiry sweep booked it as `expired_worthless` at -100% of premium. That is why the 25 July round-trips read as -$8,801 when the fills show ≈-$955, and why sold positions kept occupying the per-bucket entry caps until expiry (the SWING 12/8 / SHORT_SWING 6/6 jam that throttled entries all July and blocked them 7/31–8/13). Closing fills now resolve lots through the candidate's option contract, allocate FIFO across lots with per-lot P&L, keep a partially closed lot open (it still holds contracts) until fully closed, derive each leg's price from the broker's cumulative figures, and keep a per-order ledger so a re-polled or duplicated fill allocates nothing. The nightly close-of-books first attributes any recorded sell fill the live path missed, then the expiry sweep books only the unsold remainder — a sold lot is never swept.
- **`orion.jobs.repair_journal_expired_worthless`** — one-off operator command that repairs the 2026-08-13 sweep from the manifest (`--manifest logs/expired_journal_manifest_20260813.csv`, dry run by default, `--apply` to commit) in a single transaction, printing per-row before/after; it refuses to apply if any lot would be left open.
- **Monday-open entries are no longer blocked by a weekend "discovery degraded" false positive.** Ticker discovery looks back 24 hours in `bronze_events`; every weekend the last row aged past that window (Saturday ~20:00 UTC), the empty result was scored as `static_fallback`, the degradation streak climbed for ~40 hours, and `system_status.degraded_discovery` was DEGRADED when Monday opened — so `ExecutionEngine` rejected every new entry (`EXECUTION BLOCKED: Ticker discovery is DEGRADED`) until the first bronze rows landed roughly five minutes into the session. A successful-but-empty bronze read while the exchange calendar says the market is closed is now reported as `market_closed_idle`: it still hands back the static ticker list, but it resets the streak, persists OK, and logs once at INFO on the transition. The fence during a session is unchanged — an empty bronze read while the market is open is still `static_fallback` and still trips DEGRADED at streak 3 — and an unreadable calendar is treated as open so it can never silence the fence.
- **The documented per-bucket exit barriers are now the exit policy actually applied when no exit model is loaded.** No `*_exit.pkl` has been deployed since 2026-07-01, yet the position monitor still asked the exit classifier for a verdict on every position the deterministic barriers passed — and with nothing loaded the classifier answered with its built-in heuristic, whose stops (SWING -20% vs the documented -40%, plus a 48-hour wall-clock hold and a peak-giveback trailing stop) are tighter than the barriers and therefore always fired first. In July that heuristic stopped out 9 positions for -$1,131, five of them within 30 minutes of entry on bid/ask noise. The classifier is now consulted only for a bucket that has a trained model loaded; otherwise the barriers in `DEFAULT_BUCKET_PARAMS` (and `ORION_EXIT_BUCKET_OVERRIDES`) are the whole policy. Behaviour with a loaded model is unchanged. This fails closed: if the barrier evaluation itself raises (for example a malformed override entry), it is retried with the unoverridden bucket defaults, and only if that also raises does the monitor page `EXIT_POLICY_EVALUATION_FAILED` and consult the classifier as the last resort for that position — a position is never left with no exit policy.
- **A position whose entry lookup times out now gets its exit parameters from the contract, not a SWING default frozen for the life of the process.** The 2-second entry-context fetch used to fall back to `bucket=SWING` on timeout and cache that fallback permanently — in July, 85 timeouts left 0DTE contracts running SWING cadence, SWING stops, and no 0DTE no-progress exit or flatten. The bucket is now derived from the OCC symbol's expiry (0DTE / SHORT_SWING / SWING / POSITION by current DTE, matching the existing no-row fallback), the expiry date is set from the symbol so the time-stop is armed, and the timeout is not cached: the lookup is retried on the next cycle, and when it succeeds the real bucket, entry time, and decision are applied to the already-tracked position without disturbing its running peak/trough. Non-option symbols keep the SWING default.
- **`exit_decisions` now records which exit rule actually fired.** Every exit routed through the monitor was relabelled `ml_exit_<bucket>` before being persisted (all 39 July rows), so stop-loss, profit-target, expiry, and flatten exits were indistinguishable in the audit trail. The rule's own `rule_id` is now persisted; classifier-driven exits keep the `ml_exit_<bucket>` label. Close routing is unchanged — a monitor exit is still submitted with `IMMEDIATE` urgency — and the row is deliberately not linked to the entry `candidate_id` at submission, because the position manager treats such a link as "closed" while the close order may still be unfilled.

- **Ingestion no longer takes 7-13 minutes to restart**: Heber silver reads with a start time (bars, flow, darkpool, market tide, greek exposure, max pain, IV rank) now skip `dt=` partitions older than that start date instead of opening every parquet file ever synced. Feature-engine history hydration asks for a 5-day window per ticker, which was previously scanning 183k+ files (back to 2024) per ticker at startup. Reads with no start time still cover the full history.
- **Ingestion's single-instance lease no longer goes stale over weekends**: the market-closed sleep loop now renews the `service_lease_ingestion` row every 60-second chunk alongside the heartbeat. Previously the lease's last renewal froze at Friday close (observed 2026-08-14 21:40Z), so the split-brain guard was void for the entire weekend even though the health row stayed fresh.
- **ML outcome feedback is recorded again on every exit**: `log_outcome` no longer fails with asyncpg's `AmbiguousParameterError` ("integer versus double precision"). The return-percent bind is declared as a float so both of its uses in the `ml_predictions` UPDATE agree, and the outcome timestamp uses the portable `CURRENT_TIMESTAMP` so the same statement runs on the SQLite test database. Until now every closed position logged "Failed to log outcome" and prediction accuracy never received realized results.
- **Heber-polled flow prints are now timestamped at print time, not at ingest time.** Heber Silver `flow_alerts` rows carry the print time in `ts_event` — the only event-time column in that parquet schema. The flow normalizer only looked for `timestamp` / `created_at` / `flow_ts_utc`, none of which a Heber row has, so every poll-path (`heber_flow`) event silently got the normalization wall-clock as its `flow_ts_utc`. The rule-time signal-age gate prefers that field, so it saw ~0-2s for prints that were really 90-480s old, minted 0DTE candidates that preflight was guaranteed to reject as "Data Lag" (42 of 49 August 0DTE Data-Lag rejects; the 7 push-path ones carried the correct age), and left the flow/bar time-correlation off by the poll delay. The normalizer now reads `ts_event`; Gateway-push payloads (`timestamp`) and the legacy `flow_ts_utc` key keep working. The ingest-time born-stale drop already used the raw `ts_event` and is unchanged.
- **The "daily" loss limit now actually resets each trading day.** `current_daily_loss` — the input to the `max_daily_loss` kill switch — was only ever changed by fills, reloaded verbatim on every restart, and zeroed only by the one-time operator baseline, so nothing rolled it at a day boundary: it was a cumulative net-realized-loss total since the last baseline. Logs show it accumulating from 7/01 through 7/31 and reloading unchanged across days; at the corrected x100 accounting scale, roughly four net full-loss $500 trades would have halted every new entry permanently with `RISK REJECT: Daily Loss Limit` and no sanctioned way to clear it — indistinguishable from an outage. The persisted `risk_state` row now carries the America/New_York trading date the figure belongs to (`daily_loss_date`, migration `b6_risk_daily_loss_date`). On load, before the admission check reads the figure, and before a fill accumulates into it, a date that is not today's trading date starts the figure at zero (logged as `RISK_DAILY_LOSS_ROLLOVER` with the discarded amount and its date); a row dated today — a mid-day restart — keeps the running total. A row with no date (written before this change) takes its day identity from when it was last written: last written today, the figure is kept in force under today's date (so adding the column mid-session can never clear a limit that is already hit — at worst it also carries earlier sessions' losses until the next rollover, which only blocks more); last written on an earlier day, it starts today at zero. Realized P&L is booked into the daily figure by the broker's fill time, not by when Orion happened to process it: a fill from an earlier session that is recovered late (after a restart or a poll miss) still moves equity and positions but neither buys headroom under today's limit nor consumes it (logged as `RISK_LATE_FILL_PRIOR_SESSION`). The operator baseline command stamps the date alongside the zeroed figure. Equity, peak equity, the drawdown kill switch, and the accounting-version gate are untouched. **Operator action before restarting `execution` / `position_monitor` on this code** — the live database is managed by `init_db` `create_all` plus manual ALTERs (its `alembic_version` table is empty), so run: `ALTER TABLE risk_state ADD COLUMN IF NOT EXISTS daily_loss_date DATE;` — until it exists, `RiskManager.initialize()` cannot read the row and stays fail-closed (no entries admitted), and every risk-state save fails loudly.
- **The "daily" loss limit now actually resets each trading day.** `current_daily_loss` — the input to the `max_daily_loss` kill switch — was only ever changed by fills, reloaded verbatim on every restart, and zeroed only by the one-time operator baseline, so nothing rolled it at a day boundary: it was a cumulative net-realized-loss total since the last baseline. Logs show it accumulating from 7/01 through 7/31 and reloading unchanged across days; at the corrected x100 accounting scale, roughly four net full-loss $500 trades would have halted every new entry permanently with `RISK REJECT: Daily Loss Limit` and no sanctioned way to clear it — indistinguishable from an outage. The persisted `risk_state` row now carries the America/New_York trading date the figure belongs to (`daily_loss_date`, migration `b6_risk_daily_loss_date`). On load, before the admission check reads the figure, and before a fill accumulates into it, a date that is not today's trading date starts the figure at zero (logged as `RISK_DAILY_LOSS_ROLLOVER` with the discarded amount and its date); a row dated today — a mid-day restart — keeps the running total. A row with no date (written before this change) takes its day identity from when it was last written: last written today, the figure is kept in force under today's date (so adding the column mid-session can never clear a limit that is already hit — at worst it also carries earlier sessions' losses until the next rollover, which only blocks more); last written on an earlier day, it starts today at zero. Realized P&L reaches the daily figure fail-closed: a loss always counts against today's limit, whatever session it came from; a gain lowers today's figure only when the broker's fill time proves it happened this session (timezone-aware, not after Orion's clock, on today's New York date) — a gain from an earlier session recovered late after a restart or poll miss, or one with a missing, naive or future timestamp, still moves equity and positions but buys no admission headroom (logged as `RISK_FILL_OUTSIDE_SESSION` with the reason). The operator baseline command stamps the date alongside the zeroed figure. Equity, peak equity, the drawdown kill switch, and the accounting-version gate are untouched. **Operator action before restarting `execution` / `position_monitor` on this code** — the live database is managed by `init_db` `create_all` plus manual ALTERs (its `alembic_version` table is empty), so run: `ALTER TABLE risk_state ADD COLUMN IF NOT EXISTS daily_loss_date DATE;` — until it exists, `RiskManager.initialize()` cannot read the row and stays fail-closed (no entries admitted), and every risk-state save fails loudly.
- **Rule-based exits now use broker truth after restart**: `PositionManager` reconciles its local executed-without-exit rows against the latest confirmed Orion-owned Gateway positions before evaluating exits. Stale journal decisions and duplicate records can no longer trigger phantom close attempts; an unavailable Gateway snapshot defers this exit path until reconciliation succeeds.
- **Cross-version scikit-learn artifacts now fail over safely**: pickle loading promotes `InconsistentVersionWarning` to a load failure, so an artifact trained under an incompatible scikit-learn release is rejected and Orion uses its existing heuristic fallback instead of scoring with an unsupported deserialization.
- **Retired orphan-close automation can no longer be installed accidentally**: the 2026-05-22 emergency launchd job is stored under its documented `.DISABLED-260526` filename, outside the `*.plist` install set, with a regression check. Current operator docs now match the four native daemons, three Docker support services, active watchdog/rotation jobs, API routes, settings classes, and deleted LLM/RAG services.
- **Fill idempotency now fails closed when its database ledger is unavailable**: processed-fill lookups, fill-record writes, and processed-marker writes propagate after logging/retries instead of converting an unknown or failed write into apparent success. This prevents a database outage from authorizing duplicate fill accounting or leaving a fill permanently marked complete without its fill row. Broker-cancel cleanup failures are now logged without blocking a risk-reducing close.
- **Local quality gates match the current shared runtime**: logging tests now drain `empire-core`'s asynchronous listener before inspecting captured JSON, async failure tests use context-manager-shaped doubles without leaking un-awaited coroutines, and `mypy .` excludes the separately packaged `ci/empire_core` fixture just as CI and pre-commit already do. The full Orion type check is clean instead of reporting unrelated errors from the vendored package copy.
...
- **Exits now record the market they closed into, so what a close actually costs is finally measurable.** Entries have persisted their decision-time quote for a while, but the exit side recorded nothing — with only a fill price, there was no way to tell a wide spread apart from an exit rule firing on a stale mark, and slippage past a stop was unknowable. Every options close now stores an `exit_quote` (bid, ask, mid, spread, the mark the exit rule evaluated, timestamp, and where the quote came from) on its `exit_decisions` row, reusing the quote the close already fetched to price its limit — no extra data call, no added wait anywhere on the close path. Escalated closes record it too, since those are the expensive ones and leaving them out would have made the numbers look better than reality. When no fresh quote is available the close prices off the tracked mark exactly as before and the record says so, rather than inventing a market. This is data only: a failure to capture it is logged and ignored — it can never delay, block, re-route, or fail a close. The nightly bucket-metrics report now adds, per bucket, the median and mean realized exit slippage versus the quote mid and versus the mark the rule acted on, over closed trades that have both a captured quote and a matching fill — reported next to a coverage count showing how many closes could not be measured at all (an escalated close has no Orion-attributed fill to price it against) and how many were excluded because one order closed lots from two different buckets. It sits alongside the existing win rate and expectancy and changes no verdict and no trading behavior.
...
- **`orion.execution.risk.baseline`** — the operator command that clears the gate, plus [a runbook](docs/runbooks/risk_baseline.md). The correct equity cannot be derived from Orion's own tables (the fills ledger contains closes with no recorded entry), so a human supplies it. It is a one-time migration, not a reset button: it refuses while `execution` or `position_monitor` hold a fresh service lease, when the row is already on the current version, and when positions are open — because it zeroes `current_daily_loss` and resets `peak_equity`, which are live kill-switch inputs. `--force` overrides and logs the superseded values.

- **`--test-fire` for the launchd health probe**: `python -m orion.jobs.launchd_health_probe --test-fire` POSTs a synthetic "test-fire" alert to the configured `DISCORD_WEBHOOK_URL` (bypassing dedup) and exits non-zero if the webhook is unset or the POST fails. Lets an operator verify the notifier end-to-end on demand instead of finding out it's broken only when a real job dies. The probe's monitoring behavior is unchanged.
...
- **The halting verdict now stops new entries in that bucket by itself.** When the nightly metrics find a bucket whose trailing-50 profit factor has dropped under 0.6 (the same criterion that already flagged `consider_halting`, on at least 50 closed trades), Orion writes a durable halt for that bucket and stops opening new positions in it — previously the verdict only paged Discord and every subsequent entry went through anyway. Deliberately narrow, and reversible: it blocks **entries only**, so a halted bucket keeps closing the positions it already holds, and it is **time-boxed to ten trading sessions**, after which the nightly pass releases it and gives it ten more sessions to sample before the criterion may halt it again — a permanently halted bucket could never produce the trades that would prove it recovered, and a halted bucket takes no new entries, so on the night its window lapses the trailing fifty are still the same losing fifty that halted it. The verdict is still alerted throughout that sampling window; only the automatic action waits. It is not a kill switch: if the halt cannot be read (a database blip), Orion logs a warning and trades as it did before rather than silently stopping — the circuit breaker, daily-loss limit, and drawdown kill switch remain the hard stops and are unaffected. The halt is checked twice on the entry path — once when the candidate is screened, and again (uncached) at the moment of order submission, so a halt opened while the option chain was being priced still stops that order before any order id is minted. Halts are visible in the nightly Discord message, in the `strategy_decisions` reason (`Preflight reject: Bucket halted by measurement loop: SWING PF=0.42 n=63 until 2026-08-28`), and to `orion.jobs.bucket_halt --list`.
- **`orion.jobs.bucket_halt`** — the operator control for those halts: `uv run python -m orion.jobs.bucket_halt --list`, `--bucket SWING --set [--sessions N] [--reason "..."]` to hold a bucket by hand, `--bucket SWING --clear` to release one. A live operator halt is never released or overwritten by the nightly pass, and while it is in place the automatic halt for that bucket is suppressed — the nightly Discord message says so. It is still time-boxed by its own `--sessions`: once that window lapses it stops blocking entries and stops outranking the automatic halt, so one temporary hold can never leave a bucket permanently unprotected.

### Added

- **Automatic rotation for the logs nothing was rotating** (`scripts/rotate_native_logs.sh`, run daily at 03:30 local by a new `com.empire.orion.log-rotation` launchd agent). The structured logs have always rotated themselves; the wrapper redirects (`… >> logs/execution_native.log`) and launchd's `StandardOutPath`/`StandardErrorPath` never did, so they grew unbounded — `execution_native.log` reached 6.5 GB and the logs directory 20 GB. Rotation truncates in place rather than renaming, because a running service holds the file open and renaming would leave it writing to the renamed inode; it compresses a copy first and only truncates once the archive exists, so a failure never destroys a log. It targets an explicit allowlist so a structlog file can never be truncated out from under its own handler, skips its own output, holds a single-instance lock, and refuses to run when free disk is too low to write an archive. `--dry-run` reports without changing anything and `--self-check` verifies the logic against a throwaway directory.

### Fixed

- **Every ingestion restart tripped the global kill switch and latched it, silently blocking all order flow until someone noticed.** `HealthMonitor` starts its heartbeat clock the moment it is constructed, but startup then spends several minutes hydrating the universe and feature-engine history before the loop begins. On the market-closed path the first health check is reached (via the overnight-sleep close-of-books step) before anything publishes a heartbeat, so it read the entire startup window as ingestion having gone silent — `Heartbeat missing for 425.76s > 60.0s` — and opened the breaker. Nothing closes the breaker automatically, so a routine restart disabled trading indefinitely: after the 2026-08-13 22:02 UTC restart it stayed open for 21 hours, and the following session produced 59 candidates and 31 full-consensus decisions to execute, every one of them blocked, with zero orders placed. The heartbeat clock now starts when the loop is entered, so it measures the loop being alive rather than how long the process took to boot. The heartbeat guard itself is unchanged and still trips the breaker when it fires, and a breaker opened for any other reason still latches and still requires a human to clear it. Note what this fix does *not* claim: a wedged ingestion loop after startup was never caught by this guard and still isn't, because the loop refreshes its own heartbeat immediately before checking it. What stops trading in that case is unchanged — new entries are blocked once the health row goes stale (10 minutes), and the dead-man watchdog alerts once ingestion misses its liveness budget (5 minutes). Neither latches the breaker.
- **A latched circuit breaker now pages instead of being discovered days later.** The dead-man watchdog checked service liveness and pipeline freshness but never looked at the breaker itself, so during those 21 hours every check it ran was green — ingestion was alive and data was flowing; only order submission was dead. It now alerts whenever the breaker is OPEN, reporting the cause and how long it has been open. Deliberately not gated to market hours, because a breaker that trips overnight is exactly the one that costs a full session.
- **A restart could silently clear a genuine kill switch.** `ORION_RESET_CIRCUIT_BREAKER_ON_START` closed the global breaker during ingestion startup — an escape hatch for the false positive above, but one that made no distinction about *why* the breaker was open, so restarting after a drawdown kill switch or a manual halt would have re-armed order flow with no operator involved. Now that restarts no longer trip the breaker, the hatch was pure fail-open risk and has been removed; the setting no longer exists. Clearing the breaker is an operator action, via `python -m orion.jobs.reset_circuit_breaker` or the admin API.
- **The dead-man watchdog could fail silently.** An error in any of its stage-freshness queries aborted the whole pass before alerts were dispatched, so a broken stage table could suppress the page for a latched kill switch. The breaker is now checked first and the remaining checks are isolated: if they fail, the watchdog still reports what it knows and additionally announces that its own stall detection is degraded, rather than exiting quietly and looking healthy.
- **The circuit-breaker runbook's SQL could not run.** It referenced columns `reason` and `updated_at`; the table has `key`, `status`, `details`, and `last_updated_utc`. Corrected, and it now documents the reset script, the port-5440 gotcha, the fact that the breaker latches by design, and the restart false-positive above.
- **ITM options were riding to expiry and being auto-exercised into stock, because the time-stop was silently disarmed.** `TimeToExpiryRule` cannot evaluate a position whose `expiry_date` is `None`, and that field was only ever populated by joining back to the originating Orion decision — a join with three fallbacks that return no expiry (no matching decision, fetch error, timeout). Any position hitting one of them had *no* time-stop at all. The expiry is encoded in the OCC contract symbol itself, so it is now parsed from there whenever the database lookup comes up empty, at midnight UTC to match how `candidate_trades.expiration_date` is stored. Cost of the gap: Alpaca auto-exercised 4 `BABA260717C00105000` calls on 2026-07-17 into 400 BABA shares (strike $105 + $5.35 premium = the position's $110.35 basis), and the account shows 7 exercises and 3 assignments in total.
- **0DTE positions were never flattened on early-close days.** The 0DTE bucket has no time-to-expiry stop — a fixed 15:45 ET flatten is its only expiry protection — and on the half-day sessions NYSE closes at 13:00 ET (day after Thanksgiving, Christmas Eve) 15:45 never arrives while the market is open, so the flatten never fired and an ITM contract went to auto-exercise. The deadline is now the earlier of the configured cutoff and the session close minus 15 minutes, read from the exchange calendar, so a 13:00 close flattens at 12:45 with time for the exit to fill. Normal 16:00-close days are unchanged (still 15:45).
- **New 0DTE entries were still admitted at full size an hour before an early close.** The wind-down that blocks and shrinks 0DTE entries near the close measured the time remaining against a hardcoded 16:00 ET, so at noon on a 13:00 ET half day it believed four hours were left and applied neither the hard cutoff nor the size reduction. Both windows now count down to the real session close, so on a half day they engage three hours earlier — reducing size at 11:00 and rejecting outright at 12:00. Normal days are unchanged.
- **A failed close on a contract expiring today no longer sits out most of the session it was racing.** After five consecutive failed closes a position is abandoned for a 10-minute cooldown, which is longer than the 15-minute window between the flatten deadline and the close — a burst of transient failures could consume nearly all the runway. A position expiring today now retries after 60 seconds instead. That is keyed off the contract's expiry rather than which exit rule fired, because a 0DTE past its cutoff usually exits on stop-loss or profit target (both outrank the flatten) while racing exactly the same expiry. Contracts whose expiry has already passed are excluded — they are stale rows, not deadlines — and every other close keeps the full cooldown.
- **When the exchange calendar is unavailable, time-of-day deadlines assume the earliest close NYSE ever runs (13:00 ET) instead of 16:00**, and log a CRITICAL naming the assumption. Note this bounds the deadline but cannot flatten anything on its own: order submission is gated on the same calendar, so while it is down no exit can be sent and open 0DTE positions need a manual flatten — which is what the alert is for.
- **Equity that Orion never opened is now surfaced instead of sitting invisible.** An auto-exercise creates a stock position with no order and no fill, so it is absent from the attributed-ticker set the position adapter filters on — Orion could not see it, size against it, or exit it (its exit path is options-only). An unattributed equity position in an underlying Orion has traded options on now raises a `EXERCISE_RESIDUE_DETECTED` alert. It is advisory and never adopts the position: the account is shared, and Orion's own execution records for an exercised contract may be missing, so the alert points at the broker's `OPEXC`/`OPASN` activity feed as the authoritative check rather than asserting ownership.
- **The daily-loss and drawdown kill switches were reading option losses 100x too small.** `RiskManager.process_fill` computed realized P&L as `(exit - entry) * quantity` with no contract multiplier, but an options contract covers 100 shares — so a genuine $3,879 loss was recorded as $38.79 in `current_daily_loss` and `current_equity`, the two figures the kill switches read. Orion trades options exclusively, so this applied to every trade. The multiplier is now applied on the fill path (realized P&L, ticker exposure, and the exposure metric), which receives the OCC contract symbol and a raw per-share premium. Order admission is deliberately unchanged: `execution_engine` already passes `notional = option_price * 100`, so applying it there too would double-count. Equity symbols are unaffected. Nothing else in the codebase had the bug — `jobs/reconcile_pnl.py` and the buying-power check always applied the multiplier, which is what made the risk manager's omission visible.
- **Orion stopped placing orders entirely on 2026-07-31 because the nightly close-of-books was unreachable code.** `_run_cycle` calls `_check_overnight_sleep()` first, and that blocks the whole cycle until the next market open whenever the market is shut — but the EOD trigger sat *after* it and only fired at `hour == 1` UTC, when the market is always shut. So it never ran, on any day, ever. Its job is to realize journal rows for options that expired without a closing fill; those rows instead stayed open forever, kept counting toward the per-bucket entry caps, and eventually jammed every bucket (SWING 12/8, SHORT_SWING 6/6, SPY/QQQ/IWM 3/3). Signals kept being generated and scored the whole time — they just had nowhere to go. Close-of-books now runs on the market-closed path where it is actually reachable, once per closed session, after a 15-minute settlement grace period.
- **A failed close-of-books no longer silently counts as done.** Completion is recorded in a durable cursor (`job_cursor_state`) that advances only after a session genuinely closes, so a restart mid-run retries instead of skipping. The expiry sweep now raises on a DB error rather than returning `0` (indistinguishable from "nothing to sweep"), and a `BROKER_UNAVAILABLE` reconciliation keeps the session open for retry — both previously read as success. Sessions missed during an outage are replayed oldest-first and stop at the first failure, so a multi-night gap no longer skips every session but the latest.
- **launchd health-probe alerts silently 403'd on every send**: the probe's `_post_discord` used stdlib `urllib` with its default `Python-urllib/x.y` User-Agent, which Discord's Cloudflare edge rejects with HTTP 403 Forbidden — so no page ever reached Discord even though the webhook URL was valid and working for other repos (which post via httpx/requests). Fixed by sending an explicit `User-Agent` header. The webhook URL never needed rotating.
...
- **The LLM solver-evolution machinery (~11k LOC) is gone**: `src/orion/agents/` (meta-search agent, EOD review agent, weekly aggregator/analysis, mutation processor, proposal builder, codex client), the RAG stack (`src/orion/rag/`, `clients/trading_rag.py`, the `/search` API endpoint, `storage/models_rag.py`), the offline training stack (`ml/pattern_miner.py`, `ml/training_data.py`, `ml/model_training.py`), the drift monitor (`ml/drift_monitor.py`, `core/drift_trigger.py`), the promotion machinery (`jobs/solver_promoter.py`, `jobs/gatekeeper.py`, `jobs/run_meta_loop.py`), their entrypoints (`main_meta.py`, `main_meta_weekly.py`, `main_eod.py`, `main_pattern_miner.py`), launchd plists/wrappers, and nine docker-compose stanzas (meta-search, meta-weekly, eod-agent, dashboard-reset, pattern-miner, indexer, option_quote_tracker, nightly-backfill, quality-guardrails). None of it ever influenced a live trading decision: the promoter auto-blocked every recommendation it generated, the drift monitor never gated anything, and RAG only fed LLM prompts whose output was discarded. The measurement loop that replaces it is mechanical (realized P&L → per-bucket metrics → promote/demote thresholds). Kept: `core/promotion_rules.py` (`STAGE_ORDER` feeds the router), `ml/model_registry.py` and `ml/exit_classifier.py` (its heuristics are the live exit layer), `ml/feature_store.py`/`feature_config.py` (the live ML scorer). `main_option_quote_tracker.py` was also removed — it had been import-broken since an earlier cleanup deleted its connector.
- **Dead config**: `AgentSettings` (LLM model/AI-Gateway), `MetaSearchSettings` (scoring weights), TradingRAG and Ollama-embedding fields. The default-dev-credential warning still guards the Data-Gateway key. The launchd health probe's `REQUIRED_LABELS` no longer requires the meta-search/meta-weekly daemons (their absence would have paged a permanent false CRITICAL).
...
- **CI no longer needs a cross-repo token to resolve `empire-core`**: the test job cloned the private `empire-core` into `../empire-core` using `DATA_GATEWAY_CHECKOUT_TOKEN`, which lacks read access to that repo — so `uv sync` failed with `Authentication failed for empire-core.git` (exit 128) on every PR. Replaced the token clone with a vendored CI copy at `ci/empire_core/` (the modules Orion imports: errors, logger, ledger, http_client, config) copied into `../empire-core`. Secret-free; mirrors the Data-Gateway / 3Roses pattern. Excluded `ci/` from pre-commit.
