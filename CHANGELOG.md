...
- **Reduce-only option closes rejected as an *opening* cash-secured short put now abandon + reconcile to flat instead of escalating into a naked-short attempt (RCA 2026-06-08).** A long the broker doesn't actually hold (a phantom from a non-attributed close or a stale Gateway position read) caused the position monitor to keep trying to "close" it: Gateway `get_position` reported long, so the reduce-only guard let the limit through, but Alpaca priced the SELL as *opening* 2 cash-secured short puts and rejected it `40310000` (`required 158000 = 2×$790×100`). The old path then **escalated to the native flatten** — which would itself attempt to open a naked short — and on failure left the monitor retrying every cycle (confirmed live: 2 stranded `MU260612P00790000` exit rejections on 2026-06-08; broker `get_position` later returned `POSITION_NOT_FOUND`, proving the account was never long it). Fix: `close_position` now detects the cash-secured-put opening-short rejection signature (`_is_reduce_only_rejected_as_open`, matched on the error *text* so a genuine sibling-driven day-trading-BP exhaustion on a real long still escalates/defers as before), logs `EXIT_ABANDON_PHANTOM`, calls the new `RiskManager.reconcile_position_flat(ticker)` to drop the phantom from in-memory risk state (position count, per-ticker exposure, greeks — so sizing/greek gates aren't computed against exposure we don't hold), and returns success so the monitor stops retrying — never escalating to a flatten that could open a short. TDD in `tests/execution/test_close_position_options_rth.py`. NOTE: the underlying Gateway position-consistency issue (get_position returning a long Alpaca won't let us close) is a Data-Gateway concern tracked separately; this is the Orion-side defensive guard.
