...
- **Stale `poetry.lock` deleted**: uv is the package manager (`uv.lock` is authoritative); the Poetry lockfile survived the Poetry-to-uv migration and referenced nothing current.
- **`tests/api/test_requests_validation.py` deleted**: it tested alpaca-py's own ACH bank-relationship validation — broker functionality Orion never uses — and 4 of its 5 tests were permanently skipped.
...
- **Archived meta/labeler jobs no longer spam Discord**: the dead-man watchdog now ignores retired service-liveness rows for `meta_search`, `meta_weekly`, `eod_agent`, `meta_labeler`, and `price_target_labeler`, so stale rows left behind by the solver/labeler revamp do not page forever. The launchd-health probe also stops double-paging one-shot alert jobs that intentionally exit `2`, and treats a launchd row with a live PID as running even if launchd remembers an older non-zero exit.
