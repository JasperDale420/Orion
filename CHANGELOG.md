...
- **A dead exchange calendar no longer blocks every option exit.** The market-hours gate in front of the close path raised when the calendar failed to load, and the schedule is a process-lifetime singleton — so one failed init left every option position unexitable until the process was restarted, and an ITM contract rode to expiry to be auto-exercised into equity Orion can neither see nor sell. Option closes now fall back to the broker's own market clock, which is the authority that will accept or reject the order anyway and, unlike a wall clock, knows about holidays and early closes. That distinction matters: a plain weekday 9:30–16:00 ET window would submit at 14:00 on a session that closed at 13:00, and Alpaca queues a day order placed after the close rather than rejecting it — the exit would be recorded as complete while actually resting until the next session, and the contract would still expire unexited. The fallback fails closed on anything that isn't an unambiguous "open", caches only the "closed" answer (so the every-few-seconds retry across every open position can't flood the Gateway, while an "open" answer is never reused and so can't go stale across the close), re-checks the window immediately before each submit because the cancel sweep and quote lookup in between can themselves cross the close, and raises a throttled CRITICAL the whole time it is in force. Entries are deliberately excluded: a dead calendar still blocks new risk and only lets Orion shed it.
- **A closed options session no longer counts against a position's close-retry budget.** Every skipped close was recorded as a failed one, and five of them abandons a position for ten minutes — so an overnight retry burst (five cycles is under half a minute) exhausted the budget on every open option and abandoned each one into a cooldown that expired just after the next session opened, delaying the first real exit attempt of the day. The monitor now checks the session immediately before each individual position's attempt and treats a shut session as a deferral: nothing is submitted and nothing is charged. Closes that fail for any other reason still count, so a genuinely stuck position is still abandoned as before.
