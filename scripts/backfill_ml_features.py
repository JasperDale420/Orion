    ...
    async def query(session: Any) -> Optional[float]:
        try:
            ...
            current_result = await session.execute(current_stmt, {
                "ticker": ticker, "lookback_1h": lookback_1h, "entry_ts": entry_ts
            })
            ...
            avg_result = await session.execute(avg_stmt, {
                "ticker": ticker, "lookback_30d": lookback_30d, "lookback_1h": lookback_1h
            })
    ...
    async def query(session: Any) -> Optional[float]:
        try:
            ...
            high_result = await session.execute(high_stmt, {
                "ticker": ticker, "lookback_52w": lookback_52w, "entry_date": entry_date
            })
