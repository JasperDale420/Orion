...
def test_pattern_miner_training_source_legacy_sql_falls_back_to_heber_gold(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("ORION_PATTERN_MINER_TRAINING_SOURCE", "legacy_sql")
...
@pytest.mark.asyncio
async def test_fetch_training_data_legacy_source_still_uses_heber_without_local_db(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("ORION_ENABLE_LEGACY_LABEL_PIPELINES", "true")
    monkeypatch.setenv("ORION_ENABLE_LEGACY_PATTERN_MINER_TRAINING", "true")
...
@pytest.mark.asyncio
async def test_fetch_training_data_keeps_expired_rows_when_snapshot_metadata_absent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("ORION_ENABLE_LEGACY_LABEL_PIPELINES", "true")
    monkeypatch.setenv("ORION_ENABLE_LEGACY_PATTERN_MINER_TRAINING", "true")
