...
Production code genuinely exercised here (on Postgres, not SQLite):
  - orion.rag.vector_store.VectorStore.add_document  -> pgvector embedding_vec
    column round-trip via session.merge
  - orion.rag.vector_store.VectorStore.search        -> embedding_vec.l2_distance
    server-side ORDER BY + JSONB metadata filters + cosine re-rank
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_pgvector_round_trip_and_similarity_search(pg_db):
    """
    Insert RagDocuments with real 768-dim pgvector embeddings, then run the
    production VectorStore.search (embedding_vec.l2_distance ORDER BY) and assert
    the nearest document ranks first.

    Exercises: VectorStore.add_document (session.merge -> pgvector column write)
    and VectorStore.search (server-side l2_distance ordering + JSONB metadata
    filter + cosine re-rank) against real pgvector.
    """
    ...
    # The "query" lands at hot_index 5. doc_near shares hot_index 5 (l2 distance
    # ~0); doc_far is orthogonal (hot_index 600). The production query must rank
    # doc_near ahead of doc_far.
    query_vec = _unit_vector(EMBEDDING_DIM, 5)
    near_vec = _unit_vector(EMBEDDING_DIM, 5)
    far_vec = _unit_vector(EMBEDDING_DIM, 600)

    near_id = f"{run_tag}_near"
    far_id = f"{run_tag}_far"

    store = VectorStore()

    # Stub ONLY the embedder so vectors are deterministic. add_document(content)
    # would otherwise embed `content`; we map content -> the vector we want.
    content_to_vec = {
        "near document content": near_vec,
        "far document content": far_vec,
        "query text": query_vec,
    }

    async def fake_resolve(text_in: str) -> list[float]:
        return content_to_vec[text_in]

    store._resolve_embedding = fake_resolve  # type: ignore[method-assign]

    try:
        # Real production write path: merge into pgvector embedding_vec column.
        await store.add_document(
            doc_id=near_id,
            content="near document content",
            source_type="TEST",
            source_id=near_id,
            metadata={"ticker": ticker, "marker": TEST_MARKER},
        )
        await store.add_document(
            doc_id=far_id,
            content="far document content",
            source_type="TEST",
            source_id=far_id,
            metadata={"ticker": ticker, "marker": TEST_MARKER},
        )

        # Sanity: the vector actually persisted into the pgvector column (not
        # NULL), proving the Vector(768) bind/round-trip works on Postgres.
        async with pg_db.async_session_factory() as session:
            non_null = await session.execute(
                text("SELECT count(*) FROM vector_documents WHERE doc_id = ANY(:ids) AND embedding_vec IS NOT NULL"),
                {"ids": [near_id, far_id]},
            )
            assert non_null.scalar() == 2, "embedding_vec did not persist as a pgvector value"

        # Real production search path: l2_distance ORDER BY on Postgres, scoped
        # to our ticker so we only see this run's rows.
        results = await store.search("query text", k=10, ticker=ticker)

        result_ids = [d.doc_id for d in results]
        assert near_id in result_ids, f"near doc missing from results: {result_ids}"
        assert far_id in result_ids, f"far doc missing from results: {result_ids}"
        # The nearest neighbour must outrank the orthogonal one.
        assert result_ids.index(near_id) < result_ids.index(far_id), (
            f"l2_distance/cosine ranking wrong: near={near_id} should precede far={far_id}, got {result_ids}"
        )

        # Round-trip integrity: the stored Python-side embedding list is intact.
        near_doc = next(d for d in results if d.doc_id == near_id)
        assert isinstance(near_doc.embedding, list)
        assert len(near_doc.embedding) == EMBEDDING_DIM
    finally:
        async with pg_db.async_session_factory() as session:
            await session.execute(
                text("DELETE FROM vector_documents WHERE doc_id = ANY(:ids)"),
                {"ids": [near_id, far_id]},
            )
            await session.commit()

    # Confirm cleanup is complete (idempotent re-runs).
    async with pg_db.async_session_factory() as session:
        remaining = await session.execute(
            text("SELECT count(*) FROM vector_documents WHERE doc_id LIKE :p"),
            {"p": f"{run_tag}%"},
        )
