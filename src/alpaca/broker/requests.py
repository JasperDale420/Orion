...
class UploadW8BenDocumentRequest(NonEmptyRequest):
    ...
    def root_validator(cls, values: dict) -> dict:
        ...
        if content_is_none and content_data_is_none:
            raise ValueError("You must specify one of either the `content` or `content_data` fields")

        if not content_is_none and not content_data_is_none:
            raise ValueError("You can only specify one of either the `content` or `content_data` fields")
        ...
        if bank_code_type == IdentifierType.ABA:
            for international_param in international_parameters:
                if international_param in values and values[international_param] is not None:
                    raise ValueError(f"You may only specify the {international_param} for international bank accounts.")
        elif bank_code_type == IdentifierType.BIC:
            for international_param in international_parameters:
                if international_param not in values or values[international_param] is None:
                    raise ValueError(f"You must specify the {international_param} for international bank accounts.")
...
class OrderRequest(BaseOrderRequest):
    ...
    def order_type_must_be_market_for_lct(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (
            ...
            raise ValueError("Order type must be OrderType.MARKET if the order is in a local currency.")
        return values
    ...
    def root_validator(cls, values: dict) -> dict:
        ...
        if entry_type is not None and entry_type == JournalEntryType.SECURITY:
            ...
            if not symbol or not qty:
                raise ValueError("Security journals must contain a symbol and corresponding qty to transfer.")
