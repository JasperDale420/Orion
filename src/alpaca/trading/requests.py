...
class ClosePositionRequest(NonEmptyRequest):
    ...
    def root_validator(cls, values: dict) -> dict:
        ...
        if qty is None and percentage is None:
            raise ValueError("qty or percentage must be given to the ClosePositionRequest, got None for both.")

        if qty is not None and percentage is not None:
            raise ValueError("Only one of qty or percentage must be given to the ClosePositionRequest, got both.")
...
class UpdateWatchlistRequest(NonEmptyRequest):
    ...
    @model_validator(mode="before")
    def root_validator(cls, values: dict) -> dict:
        if ("name" not in values or values["name"] is None) and ("symbols" not in values or values["symbols"] is None):
...
class TrailingStopOrderRequest(OrderRequest):
    ...
    def root_validator(cls, values: dict) -> dict:
        ...
        if not trail_percent_set and not trail_price_set:
            raise ValueError("Either trail_percent or trail_price must be set for a trailing stop order.")
