    ...
    def __init__(self, raw_data: RawData) -> None:
        ...
        if raw_data is not None:
            for symbol, trades in raw_data.items():
                parsed_trades[symbol] = [Trade(symbol, trade) for trade in trades if trade is not None]
...
class TradingStatus(BaseModel):
    ...
    def __init__(self, symbol: str, raw_data: RawData) -> None:
        ...
        mapped = {
            TRADING_STATUS_MAPPING.get(key): val for key, val in raw_data.items() if key in TRADING_STATUS_MAPPING
        }
...
class TradeCancel(BaseModel):
    ...
    def __init__(self, symbol: str, raw_data: RawData) -> None:
        ...
        mapped = {TRADE_CANCEL_MAPPING.get(key): val for key, val in raw_data.items() if key in TRADE_CANCEL_MAPPING}
...
class TradeCorrection(BaseModel):
    ...
    def __init__(self, symbol: str, raw_data: RawData) -> None:
        ...
        """
        mapped = {
            TRADE_CORRECTION_MAPPING.get(key): val for key, val in raw_data.items() if key in TRADE_CORRECTION_MAPPING
        }
