...
class VectorStore:
    ...
    async def add_document(self, doc_id: str, content: str, source_type: str, source_id: str, metadata: dict):
        """
    ...
    async def search(
        ...
        *,
        ...
        """
        PRD 14.2: Hybrid retrieval (vector + keyword) with metadata filters.
        """
        ...
        async with async_session_factory() as db:
            stmt_base = select(VectorDocument)
            if tickers:
                normalized = [t.strip() for t in tickers if t and t.strip()]
                if normalized:
                    stmt_base = stmt_base.where(VectorDocument.metadata_json["ticker"].as_string().in_(normalized))
            elif ticker:
                stmt_base = stmt_base.where(VectorDocument.metadata_json["ticker"].as_string() == ticker)
            if doc_type:
                stmt_base = stmt_base.where(
                    (VectorDocument.metadata_json["doc_type"].as_string() == doc_type)
                    | (VectorDocument.metadata_json["entity"].as_string() == doc_type)
                )
            if rule_id:
                stmt_base = stmt_base.where(VectorDocument.metadata_json["rule_id"].as_string() == rule_id)
            if model_version:
                stmt_base = stmt_base.where(VectorDocument.metadata_json["model_version"].as_string() == model_version)
            if market_session:
                stmt_base = stmt_base.where(VectorDocument.metadata_json["session"].as_string() == market_session)
            if start_dt:
                stmt_base = stmt_base.where(
                    VectorDocument.metadata_json["timestamp"].as_string() >= start_dt.isoformat()
                )
            if end_dt:
                stmt_base = stmt_base.where(VectorDocument.metadata_json["timestamp"].as_string() < end_dt.isoformat())
            if min_premium_usd is not None:
                stmt_base = stmt_base.where(
                    cast(VectorDocument.metadata_json["premium_usd"].as_float(), Float) >= float(min_premium_usd)
                )
            if max_premium_usd is not None:
                stmt_base = stmt_base.where(
                    cast(VectorDocument.metadata_json["premium_usd"].as_float(), Float) <= float(max_premium_usd)
                )

            # Prefer server-side vector search if available; otherwise fallback to in-python ranking.
            vec_docs: list[VectorDocument] = []
            try:
                ...
                stmt_vec = (
                    stmt_base.where(VectorDocument.embedding_vec.is_not(None))
                    .order_by(VectorDocument.embedding_vec.l2_distance(query_embedding))
                    .limit(max(200, k))
                )
            ...
            stmt_kw = (
                stmt_base.where(VectorDocument.content.ilike(f"%{query}%"))
                .order_by(VectorDocument.created_at_utc.desc())
                .limit(200)
            )
            result_kw = await db.execute(stmt_kw)
            kw_docs = list(result_kw.scalars().all())

            merged: dict[str, VectorDocument] = {d.doc_id: d for d in vec_docs}
            for d in kw_docs:
