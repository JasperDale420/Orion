    ...
    def _fetch_greek_exposure(self, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if resp.status_code >= 400:
                ...
                logger.warning("Non-retryable greek exposure status=%s ticker=%s", resp.status_code, ticker)
                return None
            return resp.json()
        except requests.HTTPError as e:
            status = getattr(e.response, "status_code", None)
            if _is_retryable_gateway_status(status):
                logger.warning("Retryable greek exposure HTTP error status=%s ticker=%s", status, ticker)
                raise
            logger.warning("Non-retryable greek exposure HTTP error status=%s ticker=%s", status, ticker)
            return None
        except (requests.Timeout, requests.ConnectionError) as e:
            logger.warning(f"Transient network error fetching greek exposure for {ticker}: {e}")
            raise
    ...
    async def fetch_and_store(self, tickers: List[str]) -> int:
        ...
        for ticker in tickers:
            try:
                ...
                logger.warning(f"Greek exposure fetch exhausted retry budget for {ticker}: {e}")
                continue
